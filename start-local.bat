...
start "Apionix Portal" /D "%PORTAL_ROOT%" "%PYTHON_EXE%" "%PORTAL_ROOT%serve_static.py" --directory "%PORTAL_ROOT%" --port 8080 --bind 127.0.0.1

echo Starting APK frontend on http://127.0.0.1:5173 ...
start "APK Frontend" /D "%PLATFORM_ROOT%\FrontendUI\dist" "%PYTHON_EXE%" "%PORTAL_ROOT%serve_static.py" --directory "%PLATFORM_ROOT%\FrontendUI\dist" --port 5173 --bind 127.0.0.1
