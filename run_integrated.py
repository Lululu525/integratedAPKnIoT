...
def prepare_iot_frontend(session: dict[str, str]) -> None:
    ...
    bootstrap = (
        ...
        "if (event.origin === 'http://127.0.0.1:8080' && event.data === 'apionix:login-save') {"
        "sessionStorage.removeItem('ota.session');"
        "location.href = '/login';"
        "}"
        "});"
        "function reportApionixSession() {"
        "var stored = sessionStorage.getItem('ota.session');"
        "var guest = true;"
        "try { guest = !stored || JSON.parse(stored).username === 'apionix-demo'; } catch (_) {}"
        "if (parent !== window) {"
        "parent.postMessage({type:'apionix:session-state', guest:guest}, 'http://127.0.0.1:8080');"
        "}"
        "}"
        "setInterval(reportApionixSession, 500);"
        "addEventListener('DOMContentLoaded', reportApionixSession);"
        "</script>"
    )
